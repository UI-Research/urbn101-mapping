...
2) [**Projected coordinate systems**]()
  ...
  - Is a particular geographic coordinate system + a particular *projection*
    ...
    - All projections distort the true shape of the earth in some way. Required [xkcd comic](https://xkcd.com/977/)
...
As you can see there is an `EPSG` code and a `proj4` string, either of which can uniquely identify the CRS being used. The `EPSG` codes are usually easier to work with and are a string of 4-5 numbers that represent CRS definitions. The acronym `EPSG`, comes from the now defunct European Petrol Survey Group.
