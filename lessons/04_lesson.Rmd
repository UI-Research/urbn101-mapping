...
<font color="#55b748">**Step 2:**</font> Copy and paste the following to the top of `04_more_spatial_operations.R`. This loads in all the necessary libraries for today. With the cursor highlighting all the below text, click Control-Enter.
...
<font color="#55b748">**Step 2:**</font> Spatially join `dc_tracts` to `dc_parks` and set `join=st_intersects`. Call the joined data `parks_joined`. How many rows does `parks_joined` have? 


<font color="#55b748">**Step 3:**</font> Spatially join `dc_tracts` to `dc_parks` and set `join=st_covered_by`. Call the joined data `parks_covered`. How many rows does `parks_joined` have? 



As seen above, spatial joins  sometimes return more rows than were in the initial data. This is because some parks were located in many census tract, and each park-tract combination is returned as a row.  Let's take the example of Marvin Gaye Park
...
With `st_join` you retain the full spatial information of the left hand side variable. So if you were to look at the shape of each of the 4 rows above, they would be the exact same. Let's see that with the `st_equals()` function which checks for equality of geometries. 
...
<font color="#55b748">**Step 1:**</font> Using the same steps as above, calculate the proportion of all parks in all census tracts. 

<font color="#55b748">**Step 2:**</font> Filter your dataset to the park named "Oxon Run Park". How many tracts does it fall into? What tract has the highest proportion of the park?


<font color="#55b748">**Step 3:**</font> Make a plot of the Oxon Run Park colored in by the Census Tract the park is in.  
...
ggplot()+
  ...
  geom_sf(data = (fire_stations %>% slice(1:5)), fill = "red") 
...
<font color="#55b748">**Step 2:**</font> Calculate the minimum distance between each fire station and each DC tract. Append this information to the `fire_stations` dataframe as a column named `closest_fire_station`.

<font color="#55b748">**Step 3:**</font> Make a chloropleth map of distance to fire stations for every census tract in DC. Use ggplot() and map the colors (ie the `fill=` variable) to `closest_fire_station`. 
...
### Exercise 5: Using st_centroid()
<font color="#55b748">**Step 1:**</font> Calculate the minimum distance between each fire station and *the centroid* of each DC tract. Append this information to the `fire_stations` dataframe as a column named `closest_fire_station_to_centroid`
