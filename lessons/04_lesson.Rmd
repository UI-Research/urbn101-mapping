...
* If you want to change the opacity of the colored in regeions of the map, use the `alpha.regions` argument and choose a number from `0` to `1` (like `0.3`).
...
<font color="#55b748">**Step 2:**</font> Make an interactive map of `dc_tracts` colored in by the `"ALAND"` variable (the area of the land covered by that tract). Call this map `dc_water`.
...
<font color="#55b748">**Step 2:**</font> For the `dc_tracts` mapview statement, set `col.regions = "steelblue"` and for the `dc_parks` mapview staetment set `col.regions = "green"`
...
You can crop an object into a rectangle by manually setting the corodinates of your rectangle. But remember the coordinates need to match the CRS of the sf dataframe! So if you want to crop using latitides/longitudes, you need to set the CRS to a system that uses latitudes/longitudes
